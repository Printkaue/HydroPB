...
const larguraMapa = 5200;
const alturaMapa = 3400;
...
const baciasHidrograficas = [
     {
        ...
        imagem: "Captura de tela 2026-09-07 163329.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 164938.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165920.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165246.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165628.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165301.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165716.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 170115.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 165114.png", 
    ...
    },
    {
        ...
        imagem: "Captura de tela 2026-09-07 170132.png", 
...
function criarMarcadores() {
    ...
    baciasHidrograficas.forEach(bacia => {
        ...
        el.style.left = `${bacia.x}px`;
        el.style.top = `${bacia.y}px`;
        mapa.appendChild(el);
    });

    // Açudes
    acudesELagos.forEach(acude => {
        const el = document.createElement('div');
        el.className = 'marcador-acude';
        el.innerText = '📍';
        el.style.left = `${acude.x}px`;
        el.style.top = `${acude.y}px`;
        mapa.appendChild(el);
    });
}

function preencherTabelaAcudes() {
    const tbody = document.getElementById('tabela-acudes-body');
    if (!tbody) return;
    tbody.innerHTML = '';
    acudesELagos.forEach(item => {
        const tr = document.createElement('tr');
        tr.innerHTML = `<td>${item.municipio}</td><td>${item.nome}</td><td>${item.capacidade}</td>`;
        tbody.appendChild(tr);
    });
}

// --- CONTROLES DE TECLADO (NOTEBOOK) ---
...
// --- CONTROLE DE JOYSTICK VIRTUAL (CELULAR + TOUCH/POINTER) ---
...
joystickZone.addEventListener('pointermove', (e) => {
    if (joystickAtivo && e.pointerId === joystickPointerId) {
        atualizarJoystick(e);
    }
});
...
joystickZone.addEventListener('pointerup', soltarJoystick);
joystickZone.addEventListener('pointercancel', soltarJoystick);
...
function atualizarJoystick(e) {
    ...
    const raiolimite = rect.width / 2;
    const distancia = Math.hypot(dx, dy);

    if (distancia > raiolimite) {
        dx = (dx / distancia) * raiolimite;
        dy = (dy / distancia) * raiolimite;
    }

    joystickStick.style.transform = `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px))`;

    joystickVector.x = dx / raiolimite;
    joystickVector.y = dy / raiolimite;
}
...
function loopJogo() {
    if (jogoAtivo) {
        ...
        // Leitura do Joystick Virtual (Se ativo, sobressai o teclado)
        if (joystickAtivo) {
            dx = joystickVector.x;
            dy = joystickVector.y;
        }
        ...
        // Limites do Mapa
        posX = Math.max(15, Math.min(larguraMapa - 15, posX));
        posY = Math.max(15, Math.min(alturaMapa - 15, posY));

        // Atualizar Personagem
        nonequinho.style.left = `${posX}px`;
        nonequinho.style.top = `${posY}px`;

        // Atualizar Câmera e Proximidade
        atualizarCamera();
...
function atualizarCamera() {
    ...
    let camX = posX - camWidth / 2;
    let camY = posY - camHeight / 2;

    camX = Math.max(0, Math.min(larguraMapa - camWidth, camX));
    camY = Math.max(0, Math.min(alturaMapa - camHeight, camY));

    mapa.style.transform = `translate(${-camX}px, ${-camY}px)`;
}
...
function verificarProximidade() {
    ...
    // Verificar Bacias
    for (let bacia of baciasHidrograficas) {
        let dist = Math.hypot(posX - bacia.x, posY - bacia.y);
    ...
    // Verificar Açudes se não achou bacia
    if (!proximo) {
        for (let acude of acudesELagos) {
            let dist = Math.hypot(posX - acude.x, posY - acude.y);
            if (dist < distanciaInteracao) {
                proximo = acude;
                break;
            }
...
// --- ABRIR E FECHAR MODAIS DE INFORMAÇÃO ---
btnInspecionar.addEventListener('click', () => {
    if (!elementoAtual) return;

    modalTitulo.innerText = elementoAtual.nome;
    
    if (elementoAtual.imagem) {
        modalImagem.src = `assets/${elementoAtual.imagem}`;
        modalImagem.style.display = 'block';
    } else {
        modalImagem.style.display = 'none';
    }

    if (elementoAtual.descricao) {
        modalDescricao.innerHTML = `<p>${elementoAtual.descricao}</p>`;
    } else {
        modalDescricao.innerHTML = `
            <p><strong>Tipo:</strong> ${elementoAtual.tipo || 'Açude'}</p>
            <p><strong>Município:</strong> ${elementoAtual.municipio || 'Não Informado'}</p>
            <p><strong>Capacidade:</strong> ${elementoAtual.capacidade || 'Não Informada'}</p>
        `;
    }
...
function toggleMenu() {
    const menu = document.getElementById('botoesSecundarios');
